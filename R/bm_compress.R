...
bm_compress_pixmap <- function(x, direction, filter) {
	...
	switch(
		direction,
		v = bm_distort(x, width = bm_widths(x), height = bm_heights(x) / 2L, filter = filter),
		h = bm_distort(x, width = bm_widths(x) / 2L, height = bm_heights(x), filter = filter),
		b = bm_distort(x, width = bm_widths(x) / 2L, height = bm_heights(x) / 2L, filter = filter)
	)
}
