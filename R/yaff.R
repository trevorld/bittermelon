...
write_yaff <- function(font, con = stdout()) {
	...
	if (length(properties)) {
		...
		properties <- lapply(seq_along(keys), function(i) {
			as_yaff_property(keys[i], properties[[i]])
		})
	...
	glyphs <- unlist(lapply(code_points, as_yaff_bm_bitmap, font), use.names = FALSE)
