...
as_bm_bitmap.array <- function(
	...
	if (dim(x)[3L] == 2L) {
		...
		m <- matrix(grDevices::rgb(grey, grey, grey, alpha = alpha), nrow = nrow(x), ncol = ncol(x))
		...
		# RGB or RGBA
		as_bm_bitmap.bm_pixmap(as_bm_pixmap.array(x), mode = mode, threshold = threshold)
	}
}
...
#' @rdname as_bm_bitmap
#' @export
as_bm_bitmap.bm_pixmap <- function(
	x,
	...,
	mode = c("alpha", "darkness", "brightness"),
	threshold = 0.5
	...
	opaque <- switch(
		...
		brightness = hex2brightness(as.character(x))
	) >=
		threshold
...
#' }
#' @export
as_bm_bitmap.character <- function(
	x,
	...,
...
as_bm_object_character <- function(
	...
	if (direction_type == "h") {
		...
		if (compose) {
			bml <- bm_compose(bml, pua_combining)
		}
		...
		if (compose) {
			l <- lapply(l, bm_compose, pua_combining)
		}
...
#' @rdname as_bm_bitmap
#' @export
`as_bm_bitmap.magick-image` <- function(
	x,
	...,
	mode = c("alpha", "darkness", "brightness"),
	threshold = 0.5
	...
	mode <- match.arg(mode)
	as_bm_bitmap.bm_pixmap(`as_bm_pixmap.magick-image`(x), mode = mode, threshold = threshold)
}
...
#' @rdname as_bm_bitmap
#' @export
as_bm_bitmap.nativeRaster <- function(
	x,
	...,
	mode = c("alpha", "darkness", "brightness"),
	threshold = 0.5
	...
	mode <- match.arg(mode)
	as_bm_bitmap.bm_pixmap(as_bm_pixmap.nativeRaster(x), mode = mode, threshold = threshold)
}
...
#' @rdname as_bm_bitmap
#' @export
as_bm_bitmap.pixmapGrey <- function(x, ..., mode = c("darkness", "brightness"), threshold = 0.5) {
	mode <- match.arg(mode)
	grey <- switch(mode, brightness = as.double(x@grey), darkness = 1 - as.double(x@grey))
	m <- flip_matrix_vertically(matrix(
		as.matrix(grey >= threshold),
		nrow = x@size[1L],
		ncol = x@size[2L]
	))
...
#' @rdname as_bm_bitmap
#' @export
as_bm_bitmap.pixmapRGB <- function(x, ..., mode = c("darkness", "brightness"), threshold = 0.5) {
	mode <- match.arg(mode)
	red <- as.double(x@red)
...
as_bm_bitmap.raster <- function(
	...
	mode <- match.arg(mode)
	as_bm_bitmap.bm_pixmap(as_bm_pixmap.raster(x), mode = mode, threshold = threshold)
}
