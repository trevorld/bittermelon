...
plot.bm_bitmap <- function(
	...
	grid::grid.newpage()
	grid::grid.raster(as.raster.bm_bitmap(x, col = col), ..., interpolate = interpolate)
}

#' @rdname plot.bm_matrix
#' @export
plot.bm_pixmap <- function(x, ..., interpolate = FALSE) {
	grid::grid.newpage()
	grid::grid.raster(as.raster.bm_pixmap(x), ..., interpolate = interpolate)
}
