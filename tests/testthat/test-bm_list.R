...
test_that("bm_overlay()", {
	...
	verify_output("txt/a_grave_overlay.txt", print(a_grave, px = px_ascii))
})
...
test_that("bm_flip()", {
	...
	verify_output("txt/capital_r_hflip.txt", print(bm_flip(capital_r, "h"), px = px_ascii))

	verify_output("txt/capital_r_bflip.txt", print(bm_flip(capital_r, "b"), px = px_ascii))

	verify_output("txt/capital_r_flip_ip.txt", print(bm_flip(capital_r, "v", TRUE), px = px_ascii))

	verify_output("txt/capital_r_hflip_ip.txt", print(bm_flip(capital_r, "h", TRUE), px = px_ascii))
})
...
test_that("bm_expand()", {
	...
	verify_output(
		"txt/capital_r_expand_2v.txt",
		print(bm_expand(capital_r, height = 2L), px = px_ascii)
	)

	verify_output(
		"txt/capital_r_expand_2vw.txt",
...
test_that("as_bm_bitmap.character()", {
	...
	verify_output(
		"txt/abbc_rltb.txt",
		print(as_bm_bitmap(c("RSTATS", "IS COOL!"), font = font, direction = "rlbt"), px = px_ascii)
	)
	...
	verify_output(
		"txt/abbc_tbrl.txt",
		print(as_bm_bitmap(c("RSTATS", "IS COOL!"), font = font, direction = "tbrl"), px = px_ascii)
	)
})
